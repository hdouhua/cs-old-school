...
Kubernetes 集群在内部维护了一个自己的专用网络，这个网络和外界隔离，要用特殊的“端口转发”方式来传递数据，还需要在集群之外用 Nginx 反向代理这个地址，这样才能实现内外沟通，对比 Docker 的直接端口映射，这里略微麻烦了一些。
...
把本地的“8080”映射到 WordPress Pod 的“80”后，kubectl 会把这个端口的所有数据都转发给集群内部的 Pod ：

```shell
kubectl port-forward wp-pod 8080:80 &
```

>注意在命令的末尾我使用了一个 & 符号，让端口转发工作在后台进行。若需要终止时，使用 fg 把后台的任务带回到前台，然后就可以简单地用“Ctrl + C”。

### 创建反向代理的 Nginx

如果不创建这个反向代理只能本机访问 (localhost / 127.0.0.1) WP 站点。
