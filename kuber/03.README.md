...
# 查看 IC 的 nginx 配置文件
k exec -it ngx-kic-dep-7f5dcf4754-qxxwt $nns -- cat /etc/nginx/conf.d/default-ngx-ing.conf

# 查看 nginx 的状态
k exec -it ngx-kic-dep-7f5dcf4754-qxxwt $nns -- curl http://localhost:8080/stub_status
```
...
```shell
k get svc,deploy,pod,cm -l tag=wp3 -o wide
#
...
# verify db
k exec -it mariadb-dep-795bfcff5c-2mvkr -- mysql -u wp -p
show databases;
```
...
```shell
k get ing,ingressclass,svc,deploy,pod,cm -l tag=wp3 -A -o wide
#
